...
const byte computer_pwm = 33; //主板pwm
...
String auth = ""; //《点灯·blinker》APP中添加设备时获得的密钥
...
const char* html_body_wificonfig = "<h1>设置</h1><form name='input' action='/' method='POST'><p>WIFI SSID 2.4G:<br><input type='text' name='ssid' value='[[ssid]]'></p><p>PASSWORD:<br><input type='text' name='pswd' value='[[pswd]]'></p><p>点灯科技 密钥:<br><input type='text' name='auth' value='[[auth]]'></p><p><input type='submit' value='保存设置'></p><p><a href='/update'>固件更新</a></p></form>";
...
BlinkerButton Button1("btn_power");
BlinkerNumber Number1("fan_fpm");
BlinkerText Text1("tex-ip");
int oState = -1;
int fanfg=0;
...
void miotQuery(int32_t queryCode)
{
    ...
    switch (queryCode)
    {
        case BLINKER_CMD_QUERY_ALL_NUMBER :
            ...
            BlinkerMIOT.powerState(computer_is_on_s);
            BlinkerMIOT.print();
            break;
...
void button1_callback(const String & state)
{
    ...
    if (state == BLINKER_CMD_BUTTON_TAP) {
        ...
        digitalWrite(computer_power_on, 1);
        delay(100);
        digitalWrite(computer_power_on, 0);
        //digitalWrite(0, 1);
        //delay(200);
        //digitalWrite(0, 0);
    }
...
void saveconfig(){
  ...
  String s_config = ssid + "|" + pswd + "|" + auth;
